...
app.post('/broadcast-test', (req, res) => {
	io.emit("message", JSON.stringify(["general", "server", "Mensaje de prueba enviado desde el servidor Electron."]));
	res.json({ ok: true });
});
...
io.on('connect', (socket) => {
	socket.on("connect_login", (msg)=> {
		...
		socket.broadcast.emit("message",JSON.stringify(["general","server", "Nuevo usuario conectado: " + msg]));
		io.emit("chatList", JSON.stringify(clients));
		io.emit("usersCount", clients.length);
		console.log(`USERS_COUNT: ${clients.length}`);
